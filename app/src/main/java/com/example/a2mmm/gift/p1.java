...
public class p1 extends View{
    ...
    public p1(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public p1(Context context, AttributeSet attrs) {
        super(context, attrs);
        //自定义属性 values/attr
    ...
    private void init(Context context){
        ...
        mArcPaint.setStrokeWidth((float) (0.3*mRadius));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mRadius>mCircleX) {
            ...
            mTextPaint.setStrokeWidth((float) (0.3*mRadius));
            //重新设置字号
            mTextPaint.setTextSize(mRadius/2);
            //重新设置外圆环宽度
            mArcPaint.setStrokeWidth((float) (0.2*mRadius));
            //重新获得字号大小
            mTextSize = (int) mTextPaint.getTextSize();
        }
        //画中心园的外接矩形，用来画圆环用
        nRadius=(float)1.3*mRadius;
        mArcRectF = new RectF(mCircleX-nRadius, mCircleY-nRadius, mCircleX+nRadius, mCircleY+nRadius);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画文字
        canvas.drawText((int)mCurrentPercent+"%", mCircleX, mCircleY+mTextSize/4, mTextPaint);
        //判断当前百分比是否小于设置目标的百分比
