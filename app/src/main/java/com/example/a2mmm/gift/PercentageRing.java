...
public class PercentageRing extends View{
    ...
    private RectF mArcRectF,ARectF,BRectF;
    private float mStartSweepValue;
    private float mTargetPercent;
    private float mCurrentPercent;

    private int mRadius;
    private float nRadius,vRadius;
    ...
    public PercentageRing(Context context, AttributeSet attrs) {
        ...
        bitmap =((BitmapDrawable) ContextCompat.getDrawable(getContext(), R.mipmap.status_circle_empty)).getBitmap();
    ...
    private void init(Context context){
        ...
        mTextPaint.setTextSize(mRadius/(float)(1.5));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        nRadius=2*mRadius;
        ARectF = new RectF(mCircleX-nRadius, mCircleY-nRadius, mCircleX+nRadius, mCircleY+nRadius);
        vRadius=(float)1.40*mRadius;
        BRectF = new RectF(mCircleX-vRadius, mCircleY-vRadius, mCircleX+vRadius, mCircleY+vRadius);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(bitmap,null,ARectF,mArcPaint);
        canvas.drawCircle(mCircleX, mCircleY, mRadius, mCirclePaint);
        //画圆环
        canvas.drawArc(BRectF, mStartSweepValue ,mCurrentAngle, false, mArcPaint);
        //画文字
        DecimalFormat decimalFormat=new DecimalFormat(".0");//构造方法的字符格式这里如果小数不足2位,会以0补足.
        String p=decimalFormat.format(mCurrentPercent);
        canvas.drawText(p+"%", mCircleX, mCircleY+mTextSize/3, mTextPaint);
        //判断当前百分比是否小于设置目标的百分比
